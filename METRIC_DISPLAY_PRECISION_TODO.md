...
- [ ] 新增统一指标格式化 API：普通数值、百分比、延迟、丢包、负载。
- [ ] 默认连续型指标保留小数点后两位，并采用四舍五入。
- [ ] 对 `null`、`undefined`、`NaN`、无效延迟等情况统一返回占位符。
- [ ] 增加单元测试覆盖四舍五入、负数、0、无效值和单位拼接。

验收：

- `npm run test -- src/utils/__tests__/metricFormat.test.ts` 通过。
